...
//save procesed images
function saveImageAsync(imageDataUrl, imageNumber) {
    const encodedImageUrl = encodeURIComponent(imageDataUrl);
    console.log('saveImageAsync-encodedImageUrl', encodedImageUrl)

    fetch(`http://localhost:3003/fetch-image?url=${encodedImageUrl}`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(blob => {
            ...
            formData.append('image', blob, `processed_image_${imageNumber}.png`);

            // Send POST request to save the image
            return fetch('http://localhost:3003/save-image', {
                method: 'POST',
                body: formData
            });
        })
...
function displayNextImage() {
    if (imageQueue.length > 0) {
        ...
        // Call function to save image asynchronously
        saveImageAsync(imageQueue[0], processedImageCounter);
    }
}
