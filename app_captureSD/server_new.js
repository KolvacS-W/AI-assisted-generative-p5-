...
// Function to handle the next request in the queue
async function handleNextRequest() {
    if (requestQueue.length > 0) {
        const request = requestQueue.shift();
        try {
            const { image_url, prompt, ws } = request;
            ...
            // Send the real-time response back to the client
...
app.post('/callfal', async (req, res, next) => {
    try {
        ...
        requestQueue.push({ image_url, prompt, ws: res });

        // You might need a way to send an acknowledgment to the client
        // res.send({ message: "Request sent. Processing..." });

        // Start processing the request if the queue was empty
